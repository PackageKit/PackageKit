...
constexpr plugin::Version PLUGIN_VERSION{ .major = PK_MAJOR_VERSION,
					  .minor = PK_MINOR_VERSION,
					  .micro = PK_MICRO_VERSION };

constexpr const char *attrs[]{ "author.name", "author.email", "description", nullptr };
constexpr const char *attrs_value[]{
	"Alessandro Astone",
	"ales.astone@gmail.com",
...
public:
	...
	NotifyPackagekitPlugin (libdnf5::plugin::IPluginData &data, libdnf5::ConfigParser &)
	    : IPlugin (data)
	{}
#else
	NotifyPackagekitPlugin (libdnf5::Base &base, libdnf5::ConfigParser &)
	    : IPlugin (base)
	{}
#endif
...
void
NotifyPackagekitPlugin::post_transaction (const libdnf5::base::Transaction &transaction)
{
	...
	try {
		...
		auto packagekitProxy = sdbus::createProxy (std::move (connection),
							   std::move (serviceName),
							   std::move (objectPath),
							   sdbus::dont_run_event_loop_thread);
		auto method = packagekitProxy->createMethodCall (std::move (interfaceName),
								 std::move (methodName));
		method << "posttrans";
...
PluginAPIVersion
libdnf_plugin_get_api_version (void)
{
...
plugin::Version
libdnf_plugin_get_version (void)
{
...
void
libdnf_plugin_delete_instance (plugin::IPlugin *plugin_object)
{
