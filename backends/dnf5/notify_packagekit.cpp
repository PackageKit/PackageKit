...
struct NotifyPackagekitData {
	...
	rpmRC open_dbus(rpmPlugin plugin, rpmts ts) noexcept
	{
		...
		try {
			...
			rpmlog(RPMLOG_DEBUG, "%s plugin: Error connecting to dbus (%s)\n", PLUGIN_NAME, e.what());
			connection.reset();
			proxy.reset();
		}
	...
	rpmRC send_state_changed() noexcept
	{
		...
		try {
			...
			rpmlog(RPMLOG_WARNING, "%s plugin: Error sending message (%s)\n", PLUGIN_NAME, e.what());
		}
...
static rpmRC
notify_packagekit_init(rpmPlugin plugin, rpmts ts)
{
...
static void
notify_packagekit_cleanup(rpmPlugin plugin)
{
...
static rpmRC
notify_packagekit_tsm_pre(rpmPlugin plugin, rpmts ts)
{
	auto *state = static_cast<NotifyPackagekitData *>(rpmPluginGetData(plugin));
...
static rpmRC
notify_packagekit_tsm_post(rpmPlugin plugin, rpmts ts, int res)
{
	auto *state = static_cast<NotifyPackagekitData *>(rpmPluginGetData(plugin));
