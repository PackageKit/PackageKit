...
static void cb_install_changed(GFileMonitor *monitor, GFile *file,
                               GFile *other_file, GFileMonitorEvent event_type,
                               PkBackend *backend) {
  pk_backend_installed_db_changed(backend);
}

static GFileMonitor *add_monitor(PkBackend *backend, const gchar *path,
                                 GCallback cb, GError *error) {
  g_autoptr(GFile) file = g_file_new_for_path(path);
  g_autoptr(GFileMonitor) monitor =
      g_file_monitor_file(file, G_FILE_MONITOR_NONE, NULL, &error);

  g_signal_connect(monitor, "changed", cb, backend);
...
void pk_backend_initialize(GKeyFile *conf, PkBackend *backend) {
  const gchar *path_installed = "/lib/apk/db/installed";
  PkApkBackendPrivate *priv = g_malloc(sizeof(PkApkBackendPrivate));
...

  {
    ...
    ctx.open_flags = APK_OPENF_READ | APK_OPENF_NO_AUTOUPDATE;
    ...
    */
     ...
     * static const char * const apk_lock_file = "lib/apk/db/lock";87
     */
  ...
  /*
     ...
     apk_istream_from_file(db->cache_fd, "installed"),
     APK_REPO_CACHE_INSTALLED);
                  }
          }
  */

  // pk_backend_job_set_locked (PkBackendJob *job, gboolean locked)
  // G_CALLBACK(cb_install_changed)
  /*
  ...
  // void pk_backend_transaction_inhibit_start ;
  // void	pk_backend_transaction_inhibit_end	;
  // gboolean	pk_backend_updates_changed		(PkBackend *backend);
  // gboolean	pk_backend_updates_changed_delay (PkBackend	*backend, guint
  // timeout);
...
void pk_backend_destroy(PkBackend *backend) {
  PkApkBackendPrivate *priv = pk_backend_get_user_data(backend);
  if (priv != NULL) {
    pk_backend_set_user_data(backend, NULL);
    g_free(priv);
  }
}
...
void pk_backend_download_packages(PkBackend *backend, PkBackendJob *job,
  ...
  OpenApkOptions options = {
      .apk_flags =
          APK_OPENF_READ | APK_OPENF_NO_STATE | APK_OPENF_NO_AUTOUPDATE,
...
void pk_backend_depends_on(PkBackend *backend, PkBackendJob *job,
...

  {
...

    {
      ...
      result =
          apk_solver_solve(&db, APK_SOLVERF_AVAILABLE, depends, &changeset);
      if (result != 0) {
        // TODO: error
      ...
      // we can't go the easy route in recursive and just print the entire
      // world, as we want to print a dependency that was already given to use
      // via the package_ids. hence swapping was deemed more correct.
      ...
      apk_array_foreach_item(change, changeset.changes) {
        ...
        apk_array_foreach_item(pkg, package_array) {
          apk_array_foreach_item(pkg_dep, pkg->depends) {
            ...
            // them multiple times
            pkg->marked = TRUE;
            convert_apk_to_package(job, &db, change.new_pkg,
...
void pk_backend_get_details(PkBackend *backend, PkBackendJob *job,
  ...
  _pk_apk_auto(apk_package_array) *pkgs = NULL;
  _pk_apk_auto(apk_string_array) *failed = NULL;
  gint result = 0;
  ...
  result = pk_apk_find_package_id(backend, job, &ctx, &db, package_ids, &pkgs,
                                  &failed);
  if (result != 0) {
    goto out;
  }
  if (apk_array_len(failed) > 0) {
    pk_backend_job_error_code(job, PK_ERROR_ENUM_INTERNAL_ERROR,
                              "apk_query_packages failed");
    goto out;
  }

  apk_array_foreach_item(package, pkgs) {
    convert_apk_to_job_details(job, package);
  }
...
void pk_backend_get_details_local(PkBackend *backend, PkBackendJob *job,
  ...
  pk_apk_query_from_files(backend, job, &ctx, &db, files, TRUE);
...
void pk_backend_get_files_local(PkBackend *backend, PkBackendJob *job,
  ...
  gint result = 0;
  OpenApkOptions options = {};
  options.apk_flags = APK_OPENF_READ | APK_OPENF_NO_AUTOUPDATE,
  ...
  pk_apk_query_from_files(backend, job, &ctx, &db, files, FALSE);
...
static void _required_by_handler(struct apk_package *pkg0,
  ...
  convert_apk_to_package(context->job, context->db, pkg0, PK_INFO_ENUM_UNKNOWN);
...
void pk_backend_get_updates(PkBackend *backend, PkBackendJob *job,
  ...
  apk_array_foreach_item(change, changeset.changes) {
    convert_apk_to_package(job, &db, change.new_pkg, PK_INFO_ENUM_NORMAL);
  }
...
void pk_backend_install_files(PkBackend *backend, PkBackendJob *job,
  ...
  gint result;
  OpenApkOptions options = {};
  gboolean simulate = pk_bitfield_from_enums(PK_TRANSACTION_FLAG_ENUM_SIMULATE);
  ...
  options.apk_flags = APK_OPENF_WRITE | APK_OPENF_NO_AUTOUPDATE;
...
struct EnumerateAvailableContext {
  PkBackendJob *job;
  struct apk_database *db;
};

static int enumerate_available(apk_hash_item item, void *pctx) {
  struct EnumerateAvailableContext *ctx = pctx;
  struct apk_package *package = item;

  convert_apk_to_package(ctx->job, ctx->db, package, PK_INFO_ENUM_UNKNOWN);
  return 0;
}
...
void pk_backend_get_packages(PkBackend *backend, PkBackendJob *job,
  ...
  struct EnumerateAvailableContext ea_ctx = {.db = &db, .job = job};
  gint result;
  ...
  if (pk_bitfield_contain(filters, PK_FILTER_ENUM_NEWEST)) {
    apk_hash_foreach(&db.available.packages, enumerate_available, &ea_ctx);
  }
  ...
  if (pk_bitfield_contain(filters, PK_FILTER_ENUM_INSTALLED)) {
    ...
    apk_array_foreach_item(package, package_array) {
      convert_apk_to_package(job, &db, package, PK_INFO_ENUM_UNKNOWN);
    }
  }
...
PkBitfield pk_backend_get_roles(PkBackend *backend) {
  ...
  roles = pk_bitfield_from_enums(
      ...
      PK_ROLE_ENUM_GET_FILES_LOCAL, PK_ROLE_ENUM_UPDATE_PACKAGES, -1);
  return roles;
}
