...
{
	PkBitfield roles;
	roles = pk_bitfield_from_enums (PK_ROLE_ENUM_CANCEL,
...
pk_backend_refresh_cache (PkBackend *backend, PkBackendJob *job, gboolean force)
{
	if (!pk_backend_is_online (backend)) {
		g_debug ("portage backend: pk_backend_is_online() == FALSE, allowing refresh to "
			 "proceed (let backend handle network errors)");
		pk_backend_job_error_code (job,
...
{
	return "Mounir Lamouri (volkmar) <mounir.lamouri@gmail.com>, Fabio Erculiani "
	       "<lxnay@sabayon.org>, Mihai Morovan <hithack9@gmail.com>";
}
